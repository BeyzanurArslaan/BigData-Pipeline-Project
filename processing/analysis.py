...
from processing.config import HDFS_PARQUET_OUTPUT_URI, RAW_DATA_DIR
...
def build_spark_session() -> SparkSession:
    """Create the Spark session used for the Phase 1 ingestion job."""
...
def load_csv_to_parquet(spark: SparkSession, csv_file: Path) -> None:
    ...
    output_uri = f"{HDFS_PARQUET_OUTPUT_URI.rstrip('/')}/{dataset_name}"
